...
for himsFiles in sorted(state):
    f_hims = []
    for (dirpath, dirnames, filenames) in walk("HMIS/" + himsFiles):
        f_hims.extend(filenames)
        break

    censusFiles = "Census/" + himsFiles
    f_census = []
    ...
    mappingYR = []
    for file1 in f_hims:
        first = file1[len(himsFiles):len(himsFiles)+4]
        print(first)
        tmp =[]
        for file2 in f_census:
            if first in file2:
                tmp.append("HMIS/" +  himsFiles+"/"+file1)
    ...
    # for i in attributes:
    #     result.pop(i)
    str = "MergedFiles/" + "Cleaned" + himsFiles + "_8to18.csv"
    result.to_csv(str,index = False)
