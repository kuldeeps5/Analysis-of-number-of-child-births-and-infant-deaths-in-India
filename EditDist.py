...
for files in mappingYR:
    ...
    listRemGuj=["_Gujarat","Aravalli","Botad","Chhotaudepur","Devbhumi Dwarka","Gir Somnath","Mahisagar","Morbi"]
    ...
    for i in rowHims_file1:
        if i not in listRemGuj:
            l = 10000000
            include = ""
            for j in rowCensus_file2:  
                ret = editDistance(str(i),str(j),len(str(i)),len(str(j)))
                if ret < l:
                    l = ret
                    include = str(j)
    ...
    ind_drop = df1[df1['Indicator'].apply(lambda x: x in listRemGuj)].index
    newdf1 = df1.drop(ind_drop)
    ###################### Drop a list of rows from Data Frame ####################
...
result.to_csv("CleanedGujarat_8to18.csv",index = False)
